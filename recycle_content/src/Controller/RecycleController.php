...
class RecycleController extends ControllerBase {
  ...
  /**
   * Constructs an RecycleController object.
   *
  ...
  public function summary() {
    ...
    foreach ($this->multiversionManager->getEnabledEntityTypes() as $entity_type_id => $entity_type) {
      ...
      $items[$entity_type_id] = [
        ...
        '#url' => Url::fromRoute('recycle_content.entity_list', ['entity_type_id' => $entity_type->id()]),
      ];
    }
    return [
      '#theme' => 'item_list',
  ...
  public function getTitle($entity = NULL) {
    if (!empty($entity)) {
      ...
      return $entity_types[$entity]->get('label') . 'recycle_content';
    }
    else {
      return 'Recycle';
    }
  ...
  public function entityList($entity_type_id = NULL) {
    ...
    return array(
      ...
      '#rows' => $rows,
      '#empty' => $this->t('The @label recycle is empty.', ['@label' => $entity_types[$entity_type_id]->get('label')]),
    );
  }
